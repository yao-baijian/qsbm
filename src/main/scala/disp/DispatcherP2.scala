...
case class DispatcherP2() extends Component {
  ...
  val pe_select           = Reg(UInt(log2Up(config.core_num) bits)) init 0
  ...
  val pe_nxt              = Reg(UInt(log2Up(config.core_num) bits)) init 0
  val pe_nxt_rdy          = Reg(Bool()) init True

  // edge read in, detect zero, decide nxt CB, RB
  val single_zero_in      = UInt(config.core_num bits)
  val partial_zero_in     = Bool()
  val partial_zero_in_r   = Reg(Bool()) init False
  val edge_in_ptr_in      = UInt(4 bits)
  val header 		          = Bool()
  val head		            = UInt(config.core_num bits)
  val header_ptr          = UInt(log2Up(config.core_num) bits)

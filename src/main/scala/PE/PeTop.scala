...
case class PeTop(config:PETopConfig) extends Component {
    ...
    val io = new Bundle {
        ...
        val raw_edge_stream     = in Vec(slave Stream (Bits(config.high_to_low_converter_config.axi_extend_width bits)), config.core_num)
    ...
    val edge_stream             = Vec(Vec(master Stream (Bits(128 / 8 bits)), config.thread_num), config.core_num)
    ...
    val high_to_low_converter       = HighToLowConvert(config.high_to_low_converter_config)
    ...
    for (i <- 0 until config.core_num) {
        ...
        for (j <- 0 until config.thread_num) {
            ...
            all_zero_table (i)(j) := edge_stream(i)(j).payload === 0 & edge_stream(i)(j).valid
        }
        ...
        for (j <- 0 until config.thread_num) {
            edge_fifo(i)(j).io.in_stream << edge_stream(i)(j)
    ...
    for (i <- 0 until config.core_num){
        ...
        for (j <- 0 until config.thread_num) {
            edge_stream(i)(j) << high_to_low_converter.io.out_edge_stream(i)(j)
        }
    }

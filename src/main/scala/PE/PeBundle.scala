...
case class PeBundle(config: PeBundleConfig) extends Component {
    ...
    val io_fifo       =  new Bundle {
        val pe1_fifo = slave Stream (Bits(config.axi_width bits))
        val pe2_fifo = slave Stream (Bits(config.axi_width bits))
        val pe3_fifo = slave Stream (Bits(config.axi_width bits))
        val pe4_fifo = slave Stream (Bits(config.axi_width bits))
        val pe5_fifo = slave Stream (Bits(config.axi_width bits))
        val pe6_fifo = slave Stream (Bits(config.axi_width bits))
        val pe7_fifo = slave Stream (Bits(config.axi_width bits))
        val pe8_fifo = slave Stream (Bits(config.axi_width bits))
    }
...
// TO DO:
// global reg need 8 pe rd address and data
        global_reg.io.rd_addr <> pe_bundle(i).io_vertex_reg.vertex_reg_addr
        global_reg.io.rd_data <> pe_bundle(i).io_vertex_reg.vertex_reg_addr

// TO DO:
// Uupdate reg need 8 pe rd address and data
...
// TO DO:
// each Bundle is connected to respective fifo
    pe_bundle(0).io_edge_fifo.edge_fifo_ready <> io_fifo.pe1_fifo.ready
    pe_bundle(0).io_edge_fifo.edge_fifo_valid <> io_fifo.pe1_fifo.valid
    pe_bundle(0).io_edge_fifo.edge_fifo_in <> io_fifo.pe1_fifo.payload
